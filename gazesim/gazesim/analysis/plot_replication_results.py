...
EXPERIMENT_NAMES = ["codevilla-screen", "codevilla-mean-mask", "codevilla-hard-mask", "codevilla-soft-mask",
                    "codevilla-dual-branch", "codevilla-random-hard-mask"][:-1]
...
def main(config):
    ...
    for e_idx, en in tqdm(enumerate(EXPERIMENT_NAMES), total=len(EXPERIMENT_NAMES), disable=True):
        ...
        results["total_mse"].append(combined_df.loc[mask, "loss/val/total/mse"].values[-1])
        results["total_l1"].append(combined_df.loc[mask, "loss/val/total/l1"].values[-1])
        ...
        for l_idx, loss in enumerate(INDIVIDUAL_LOSSES):
            ...
            results[f"{loss}_l1"].append(combined_df.loc[mask, f"loss/val/{loss}/l1"].values[-1])

    data = pd.DataFrame(results)
    data.to_csv("/home/simon/Desktop/weekly_meeting/meeting23-24/presentation_figures_and_videos/replication.csv",
                index=False)
    ...
    ax_absolute.set_ylabel("Mean test error (L1) over 3 runs")
    ax_absolute.legend()
